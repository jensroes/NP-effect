...
parameters {
  ...
  vector[E] theta_e;   // Mixing proportion by-experiment
...
transformed parameters{
  ...
  vector[E] prob_e = inv_logit(theta_e); //probability of extreme values by exp
...
model {
  ...
  for(n in 1:N){
    ...
    target += log_sum_exp(
      log_theta_e[1, exps[n]] + lognormal_lpdf(y[n] | mu + delta[exps[n]], sigmap_e[exps[n]]), 
